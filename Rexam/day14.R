...
bind_cols(group_a, group_b) # 이것저것 따지지 않고 합쳐버림.. 
...
## 조인 : 데이터 프레임을 좌우로 붙이는 방식. 
inner_join(test1, test2, by = "no")  # 두 데이터프레임을 하나로 묶는데, 일치하는 경우만, no 값이 같은 애들끼리 붙인다.
left_join(test1, test2, by = "no") # 왼쪽 데이터 프레임의 내용은 모두 출력하고, 비어있는 오른쪽 데이터 프레임은 NA 값을 준다. 
right_join(test1, test2, by = "no") # 오른쪽 데이터 프레임의 내용을 모두 출력하고, 비어있는 왼쪽 데이터 프레임은 NA 값을 준다.
full_join(test1, test2, by = "no") # 일치하는 경우가 없더라도 하나로 묶는다. 

merge(test1, test2) # 동일한 경우에만 
merge(test1, test2, all.x = T) # left_join 과 동일
merge(test1, test2, all.y = T) # right_join과 동일
merge(test1, test2, all = T) # full_join과 동일
...
# 결측치 생성
table(is.na(exam)) # 20개 행 . 5개의 열. 100개의 데이터.. 
...
sketch + geom_point() + scale_x_continuous(breaks = c(2,4,6)) + scale_y_continuous(breaks = c(15,20,25,30,35,40))
sketch + geom_line()
...
# 엑셀문서의 몇번째 시트를 읽어올 것인지 지정할 수 있다. 
...
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(shape=21, size=6, colour="blue") # color 는 테두리. fill은 채우기 색
...
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(colour="grey", shape=21, size=6, aes(fill=Price)) # 변수값으로 채우기. 
...
ggplot(Cars93, aes(x=Weight, y=MPG.highway, fill=Cylinders)) +
  geom_point(colour="grey", shape=21, size=6) + # ggplot2가 내장하고 있는 컬러로 적용됨
...
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") #396과 동일

ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") + coord_cartesian(ylim=c(60, 75))
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + coord_cartesian(ylim=c(60, 75)) # 연도 별로 채워라. 
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), colour="blue", stat="identity") + coord_cartesian(ylim=c(60, 75))
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + geom_label(aes(label=weight))+coord_cartesian(ylim=c(60, 75)) # label은 y축의 값을 보여주는 것
...
library(gridExtra)
grid.arrange(p1, p2, p1, p2, nrow = 2) # nfrow처럼 창을 나눌 때 쓰인다. 그리고자 하는 데이터를 변수에 저장한다. 
...
# 인터랙티브 그래프 만들기
ggplotly(p) # 뷰어 창에 그려진다. (html 로 그려지는 것을 뜻함.)
