...
Sys.time()
Sys.timezone()
str(Sys.Date());str(Sys.time());str(Sys.timezone()) 
...
as.Date("2020/01/01 08:00:00") - as.Date("2020/01/01 05:00:00") # as.Date(Date)형의 뺄셈 연산은 날짜까지만 표현.
...
landmesses
grep(pattern="New", x=landmesses) # 패턴에 해당하는 값의 인덱스를 추출
...
grep("ch(e|i)ck", words, value=T) # check ceick
...
fox.said
strsplit(x= fox.said, split= " ") # strsplit 쪼개기 
...
?apply
?lapply
apply(df, 1, sum, na.rm=TRUE) # 행단위
apply(df, 2, sum, na.rm=TRUE) # 열단위
lapply(df, sum, na.rm=TRUE) # 네임드 리스트.따로 방향을 명시하지 않아도, 일반적으로 열을 묶어놓은 데이터프레임이기 때문에 열방향이 일반적
sapply(df, sum, na.rm=TRUE) # 네임드 벡터
...
length(flower)
nchar(flower)
sapply(flower, function(d) if(nchar(d) > 5) return(d)) #리스트, function 을 수행했을 때 return 결과에 따라 데이터셋이 달라짐
# nchar -> 글자개수를 센다.
sapply(flower, function(d) if(nchar(d) > 5) d) # 리스트 
sapply(flower, function(d) if(nchar(d) > 5) return(d) else return(NA)) # 벡터, NULL은 들어갈 수 없다. 
sapply(flower, function(d) paste("-",d,"-", sep="")) # 벡터

sapply(flower, function(d, n=5) if(nchar(d) > n) return(d))
sapply(flower, function(d, n=5) if(nchar(d) > n) return(d), 3) #null 이 없다. 
...
rr1 <- sapply(df$w, myf, wt=F) #벡터 전달
str(rr1)

count <- 1
sapply(df, myf) # 데이터 프레임은 열단위로 전달한다.
rr2 <- sapply(df, myf)
str(rr2)
rr2[1,1]
rr2[1,"w"]
