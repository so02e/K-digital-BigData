...
# model에서의 포뮬라 식 종속변수 ~ 독립변수
model <-lm (height_son ~ height_father + height_mohter, data = height)
model 


# 결정계수와 수정된 결정계수
# Adjusted R-squared : 독립변수의 수가 많아질수록~가 되는데. 이를 반영하여. (ARS)
summary(model)

# 회귀계수 . 각각의 기울기값과 절편값... 직접 회귀식을 만들 수 있다. 
(coef_r <- coef(model))
coef_r[1]
coef_r[2]
coef_r[3]
...
summary(fdata2.lm) # temp 0.8285, 0.05보다 크기 때문에 temp를 독ㄹ립변수로 쓰는 것이 맞는지에 대한 점검이 필요하다. 
-554.5267-0.1743 * 200 + 11.8449 * 59
predict(fdata2.lm, newdata=data.frame(temp=200, pressure=59) )
cor(fdata2) # temp는 약한 상관관계를 보여준다. 


fdata3 <- read.csv("data/factory3.csv")
fdata3.lm <- lm(Y~X1+X2+X3, data=fdata3)
summary(fdata3.lm) # x2,x3는 0.05보다 큰 값들을 가지고 있기 때문에, 회귀계수가 유의미하지 않다. 전체적으로는 결정계수가 높은데 x2,x3는 무의미하다. 변수 선택 기준중의 하나인 VIF 값을 체크한다. (다중공산성? 분산 확대인자. )
...
# 방법 1. 독립변수로 쓸 변수의 이름을 모두 나열해서 작성
mtcars.lm <- lm(mpg ~ hp + wt + disp + drat, data=mtcars)
summary(mtcars.lm) # disp는 상당히 높은 p값이 나옴. 다른 변수간의 상관계수가 높기 때문에 다중공산성의 문제가 발생한 것으로 추측할 수 있다. 

# 방법 2. 남아있는 모든 변수를 독립변수로 쓸 때는 .을 써도 된다. 
...
# 2. both 방식
step(mtcars.lm, direction="both") # forward, backward를 적절하게 섞어서 쓰는 both

# 3. forward 방식
start.lm <- lm(mpg~1, data=mtcars) # 아무 변수도 선택하지 않은 상태에서 시작하고 
step(start.lm, scope=list(lower=start.lm, upper=mtcars.lm), direction="forward") # 하나하나 추가하는 형식
...
# 조정된 결정계수. .. step함수는 hp와 wt를 주장하고 있었는데, drat을 추가하면 0.82 임..
...
mtcars.lm3 <- lm(mpg ~ . - disp, data=mtcars) # .은 남아있는 모든 변수를 의미하는데 그 뒤에 - disp가 있으면, 남아 있는 모든 변수들 중에서 disp는 제외한다는 의미이다. 
AIC(mtcars.lm1, mtcars.lm2, mtcars.lm3) # AIC가 낮다는 것은 적합도가 높은 것을 뜻한다. 
...
# scale 함수는 표준화만 지원하는 함수이다. 
st_mtcars.lm <- lm(scale(mpg) ~ scale(hp) + scale(wt) + scale(disp) + scale(drat),data=mtcars)
...
summary(multico.lm) # 회귀분석을 했지만, 변수들이 모두 유의하지 않음을 보여주고 있다. 

cor(multico[-1])

library(car)
vif(multico.lm) # 분산팽창계수가 크면 클수록 그 변수는 다른 변수와 다중 공산성의 문제가 있다고 판단될 수 있다. 독립변수간의 강한 상관관계가 있는 경우. VIF 계수가 일반적으로 10보다 크면 다중공산성 문제가 있다. (엄격하게 따질경우 5) 우선 가장 큰 VIF 계수를 가진 것을 빼고, 나머지를 가지고 회귀모델을 만든다

multico.lm <- lm(Y~X1+X2+X3, data=multico)
summary(multico.lm)
vif(multico.lm)

# 각각의 변수간의 어떤 조합이 유의미한지 찾아보는 과정. 2개까지.. 
multico.regsubsets <- regsubsets(x=Y ~ . - ID, data=multico, nbest=2)
...
Sys.sleep(2)
ggmap(map)
map <- get_googlemap(center=cen, maptype="satellite",zoom=16, marker=mk) #위성지도
Sys.sleep(2)
ggmap(map)
...
map <- get_map(location=cen, maptype="toner",zoom=12, marker=mk, source="google") # 구글에서 가져올 때 
ggmap(map)
...
# 주소에 알맞은 위도 경도를 받아오는 서비스 geocode
...
mk <- geocode(enc2utf8("부산"), source = "google") # 한글 주소를 적을 때는 euc2utf8 함수를 이용해야 한다. 
cen <- c(mk$lon, mk$lat)
map <- get_googlemap(center=cen, maptype="roadmap",zoom=11, marker=mk)
ggmap(map)
...
addr <- c("제주시 용두암길 15",
          ...
          "서귀포시 색달동 산1-2",
          "제주시 한경면 고산리 125")
gc <- geocode(enc2utf8(addr)) # geocode 는 파일에 저장해서 쓰자~!
gc
...
?colorFactor # 리플렛에서 제공하는 칼라 펙터.. 리턴값이 함수이다. 
