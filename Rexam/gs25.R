...
repeat{
  ...
  Sys.sleep(1)
  curr_PageElem <- remDr$findElement(using='css','#contents > div.cnt > div.cnt_section.mt50 > div > div > div:nth-child(5) > div > span > a.on') # 선택된 페이지는 on이라는 클래스 속성이 추가된다.
  
  curr_PageNewNum <- as.numeric(curr_PageElem$getElementText()) # 현재 페이지
  cat(paste(curr_PageOldNum, ':', curr_PageNewNum,'\n'))
  
   # 1페이지에서 next 누르면 2페이지..
  # 마지막페이지에서 next누르면 마지막 페이지의 숫자와 똑같음 
...
repeat{
  ...
  if(length(nextPage$getElementAttribute("onclick"))==0){ # onclick 속성이 사라짐.
    cat("종료\n")
    break
  }
