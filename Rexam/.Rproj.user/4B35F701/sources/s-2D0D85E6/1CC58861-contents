...
exam %>% filter(class == 1) %>% select(english) # 1번학생의 결과 중 영어 성적만 꺼내겠다.
# 가독성 있게 줄 바꾸기
exam %>% 
...
str(iris)
iris %>% pull(Species) # 벡터로 만들고 싶을 때는 사용한다.
iris %>% select(Species) # 데이터 프레임 구조를 유지한다. 
...
iris %>% select(starts_with("petal")) 
iris %>% select(starts_with("petal", ignore.case=F)) # 대소문자를 반영하여 
...
# Select columns whose name maches a regular expression 정규표현식....... 변수명에 t가 들어감. 
...
iris %>% select(matches("^p")) %>% head()
iris %>% select(starts_with("p")) %>% head()
iris %>% select(matches("[Ps]e[tp]")) %>% head()
iris %>% select(starts_with("[Ps]e[tp]")) %>% head()
...
exam %>% summarise(행 = n()) # n 함수를 이용하여 행의 개수를 세고, '행'변수에 담는다.
exam %>% tally()
exam %>%  count()
